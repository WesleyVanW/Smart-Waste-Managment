...
static int16_t _hum;
 static int read(const sht31_dev_t *dev)
{
    /* read both sensor values */
    int res = sht31_read((sht31_dev_t *)dev, &_temp, &_hum);
    if (res != SHT31_OK) {
        return res;
    }
 ...
 static int read_temp(const void *dev, phydat_t *data)
{
    ...
    if (_temp_valid || read(dev) == SHT31_OK) {
...
{
    /* either local variable is valid or fetching it was successful */
    if (_hum_valid || read(dev) == SHT31_OK) {
        /* mark local variable as invalid */
        _hum_valid = false;
         data->val[0] = _hum;
 ...
 const saul_driver_t sht31_saul_driver_temperature = {
    .read = read_temp,
    .write = saul_notsup,
    .type = SAUL_SENSE_TEMP
};
 const saul_driver_t sht31_saul_driver_humidity = {
    .read = read_hum,
    .write = saul_notsup,
    .type = SAUL_SENSE_HUM
};